...
class SpellWord(Task):
    ...
    def __init__(self):
        ...
        self.task_completed_desc = "done spelling as much of \"{word}\" as you can."
    ...
    def reset(self, env):
        ...
        word_letters = [l.upper() for l in word]

        n_subword_letters = np.random.randint(3, min(len(word), 5) + 1)
        n_distractor_letters = np.random.randint(1, 3)   # 1, 2

        subword_letters_idx = np.sort(random.sample(range(len(word)), n_subword_letters))
        subword_letter_names = [word_letters[i] for i in subword_letters_idx]
        distractor_letter_names = [l for l in alphabet if l not in word_letters]
        distractor_letter_names = random.sample(distractor_letter_names, n_distractor_letters)

        # word = 'blue'
        # word_letters = [l.upper() for l in word]
        # n_subword_letters = 3
        # n_distractor_letters = 2
        # subword_letter_names = ['L', 'U', 'E']
        # distractor_letter_names = ['Z', 'Y']
        ...
        subword_letters = []
        self.letter2id = {}
        for i in range(n_subword_letters):
            pose = self.get_random_pose(env, letter_size, obstacle_pos)
            pose = (pose[0], rot)
            fname = f'{subword_letter_names[i]}.obj'
            ...
            self.letter2id[subword_letter_names[i]] = letter_id
            if os.path.exists(urdf):
                os.remove(urdf)
            subword_letters.append((letter_id, (0, None)))
        self.subword_letters = subword_letters
        ...
        # Goal: spelling as much of {word} as you can
        bottom_left_corner_pose = (bottom_left_corner_pos, (0, 0, 0, 1))

        place_y = [i * 0.1 for i in range(n_subword_letters)]
        relative_pos = [(0, y, 0.01) for y in place_y]
        targets = [(utils.apply(bottom_left_corner_pose, relative_pos[i]), rot) for i in range(n_subword_letters)]
        self.targets = targets

        for i in range(n_subword_letters):
            self.goals.append(([subword_letters[i]], np.ones((1, 1)), [targets[i]],
                               False, True, 'pose', None, 1 / n_subword_letters))
            if i == 0:
                self.lang_goals.append(self.lang_template.format(pick=subword_letter_names[i],
                                                                 place='bottom left corner'))
            else:
                self.lang_goals.append(self.lang_template.format(pick=subword_letter_names[i],
                                                                 place='right of ' + subword_letter_names[i - 1]))

        self.max_steps = n_subword_letters + 1
        self.high_level_lang_goal = 'spell as much of "{}" as you can'.format(word)

    def success(self):
        letter_poses = [p.getBasePositionAndOrientation(lid) for lid, _ in self.subword_letters]
        matches = [self.is_match(letter_poses[i], self.targets[i], symmetry=0) for i in range(len(self.subword_letters))]
        return all(matches)
