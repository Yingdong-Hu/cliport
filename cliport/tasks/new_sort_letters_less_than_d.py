...
class SortLettersLessThanD(Task):
    ...
    def __init__(self):
        ...
        self.task_completed_desc = "done sorting the letters less than \"D\" according to ASCII to the bottom side."

    def reset(self, env):
        super().reset(env)

        alphabet = utils.ALPHABET
        all_less_than_d_letters = ['A', 'B', 'C']

        n_less_than_d_letters = np.random.randint(1, 4)       # 1, 2, 3
        n_distractor_letters = np.random.randint(2, 4)        # 2, 3

        less_than_d_letter_idx = np.sort(random.sample(range(len(all_less_than_d_letters)), n_less_than_d_letters))
        less_than_d_letter_names = [all_less_than_d_letters[i] for i in less_than_d_letter_idx]
        distractor_letter_names = [l for l in alphabet if l not in all_less_than_d_letters]
        ...
        less_than_d_letters = []
        self.letter2id = {}
        for i in range(n_less_than_d_letters):
            pose = self.get_random_pose(env, letter_size, obstacle_pos)
            pose = (pose[0], rot)
            fname = f'{less_than_d_letter_names[i]}.obj'
            ...
            self.letter2id[less_than_d_letter_names[i]] = letter_id
            if os.path.exists(urdf):
                os.remove(urdf)
            less_than_d_letters.append((letter_id, (0, None)))
        self.less_than_d_letters = less_than_d_letters
        ...
        targets = [(utils.apply(bottom_side_pose, relative_pos[i]), rot) for i in range(n_less_than_d_letters)]
        self.targets = targets

        for i in range(n_less_than_d_letters):
            self.goals.append(([less_than_d_letters[i]], np.ones((1, 1)), [targets[i]],
                               False, True, 'pose', None, 1 / n_less_than_d_letters))
            if i == 0:
                self.lang_goals.append(self.lang_template.format(pick=less_than_d_letter_names[i],
                                                                 place='bottom side'))
            else:
                self.lang_goals.append(self.lang_template.format(pick=less_than_d_letter_names[i],
                                                                 place='right of ' + less_than_d_letter_names[i - 1]))

        self.max_steps = n_less_than_d_letters + 1
        self.high_level_lang_goal = 'sort the letters less than "D" according to ASCII to the bottom side'

    def success(self):
        letter_poses = [p.getBasePositionAndOrientation(lid) for lid, _ in self.less_than_d_letters]
        matches = [self.is_match(letter_poses[i], self.targets[i], symmetry=0) for i in range(len(self.less_than_d_letters))]
        return all(matches)
