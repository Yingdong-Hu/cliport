...
class PutLettersReverseAlphabeticalOrder(Task):
    ...
    def __init__(self):
        ...
        self.task_completed_desc = "done putting the letters on the tables in reverse alphabetical order."
    ...
    def reset(self, env):
        ...
        letter_names = sorted(letter_names, reverse=True)
        ...
        for i in range(n_letters):
            self.goals.append(([letters[i]], np.ones((1, 1)), [targets[i]],
                               False, True, 'pose', None, 1 / n_letters))
            if i == 0:
                self.lang_goals.append(self.lang_template.format(pick=letter_names[i],
                                                                 place='bottom left corner'))
            else:
                self.lang_goals.append(self.lang_template.format(pick=letter_names[i],
                                                                 place='right of ' + letter_names[i - 1]))

        self.max_steps = n_letters + 1
        self.high_level_lang_goal = 'put the letters on the tables in reverse alphabetical order'

    def success(self):
        letter_poses = [p.getBasePositionAndOrientation(lid) for lid, _ in self.letters]
        matches = [self.is_match(letter_poses[i], self.targets[i], symmetry=0) for i in range(len(self.letters))]
        return all(matches)
