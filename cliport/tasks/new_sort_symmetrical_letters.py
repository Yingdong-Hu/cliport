...
class SortSymmetricalLetters(Task):
    ...
    def __init__(self):
        ...
        self.task_completed_desc = "done sorting the geometrically vertically symmetrical letters to the bottom side."

    def reset(self, env):
        super().reset(env)

        alphabet = utils.ALPHABET
        all_symmetrical_letters = ['A', 'H', 'I', 'M', 'O', 'T', 'U', 'V', 'W', 'X', 'Y']

        n_symmetrical_letters = np.random.randint(2, 5)       # 2, 3, 4
        n_distractor_letters = np.random.randint(2, 4)        # 2, 3

        symmetrical_letter_idx = np.sort(random.sample(range(len(all_symmetrical_letters)), n_symmetrical_letters))
        symmetrical_letters_letter_names = [all_symmetrical_letters[i] for i in symmetrical_letter_idx]
        distractor_letter_names = [l for l in alphabet if l not in all_symmetrical_letters]
        ...
        symmetrical_letters = []
        self.letter2id = {}
        for i in range(n_symmetrical_letters):
            pose = self.get_random_pose(env, letter_size, obstacle_pos)
            pose = (pose[0], rot)
            fname = f'{symmetrical_letters_letter_names[i]}.obj'
            ...
            self.letter2id[symmetrical_letters_letter_names[i]] = letter_id
            if os.path.exists(urdf):
                os.remove(urdf)
            symmetrical_letters.append((letter_id, (0, None)))
        self.symmetrical_letters = symmetrical_letters
        ...
        place_y = [i * 0.1 for i in range(n_symmetrical_letters)]
        relative_pos = [(0, y, 0.01) for y in place_y]
        targets = [(utils.apply(bottom_side_pose, relative_pos[i]), rot) for i in range(n_symmetrical_letters)]
        self.targets = targets

        for i in range(n_symmetrical_letters):
            self.goals.append(([symmetrical_letters[i]], np.ones((1, 1)), [targets[i]],
                               False, True, 'pose', None, 1 / n_symmetrical_letters))
            if i == 0:
                self.lang_goals.append(self.lang_template.format(pick=symmetrical_letters_letter_names[i],
                                                                 place='bottom side'))
            else:
                self.lang_goals.append(self.lang_template.format(pick=symmetrical_letters_letter_names[i],
                                                                 place='right of ' + symmetrical_letters_letter_names[i - 1]))

        self.max_steps = n_symmetrical_letters + 1
        self.high_level_lang_goal = 'sort the geometrically vertically symmetrical letters to the bottom side'

    def success(self):
        letter_poses = [p.getBasePositionAndOrientation(lid) for lid, _ in self.symmetrical_letters]
        matches = [self.is_match(letter_poses[i], self.targets[i], symmetry=0) for i in range(len(self.symmetrical_letters))]
        return all(matches)
