...
class StackBlocksWarmColors(Task):
    ...
    def reset(self, env):
        ...
        n_cool_blocks = np.random.randint(2, 4)
        ...
        for i in range(n_warm_blocks - 1):
            self.goals.append(([warm_blocks[i + 1]], np.ones((1, 1)), [targs[i]],
                               False, True, 'pose', None, 1 / (n_warm_blocks - 1)))
            self.lang_goals.append(self.lang_template.format(pick=selected_warm_color_names[i + 1],
        ...
        self.max_steps = len(warm_blocks) + 1
